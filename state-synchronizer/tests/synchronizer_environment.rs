...
use state_synchronizer::{
    ...
    state_synchronizer::{StateSynchronizer, StateSynchronizerClient, SynchronizationState},
};
use std::{
    cell::{Ref, RefCell},
...
impl SynchronizerPeer {
    ...
    pub fn wait_until_initialized(&self) {
        block_on(self.synchronizer.as_ref().unwrap().wait_until_initialized()).unwrap();
    }
}
