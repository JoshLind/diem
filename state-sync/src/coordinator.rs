...
pub(crate) struct StateSyncCoordinator<T> {
    ...
    highest_known_ledger_info: Option<LedgerInfoWithSignatures>,
...
impl<T: ExecutorProxyTrait> StateSyncCoordinator<T> {
    pub fn new(
        ...
        Ok(Self {
            ...
            sync_request: None,
            highest_known_ledger_info: None,
    ...
    fn calculate_new_known_version_and_epoch(
        ...
        let new_version = self
            ...
            .ok_or_else(|| {
                Error::IntegerOverflow("The new version to request has overflown!".into())
            })?;
        ...
        if let Some(ledger_info) = ledger_info {
                ...
                && ledger_info.ledger_info().ends_epoch()
            {
                ...
                new_epoch = self
                    ...
                    .checked_add(1)
                    .ok_or_else(|| {
                        Error::IntegerOverflow("The new epoch to request has overflown!".into())
                    })?;
            }
        }
    ...
    /// Calculates the next chunk request target to create and sends the request.
    fn send_chunk_request(&mut self, known_version: u64, known_epoch: u64) -> Result<(), Error> {
    ...
    fn create_highest_available_chunk_target(&self) -> TargetType {
        TargetType::HighestAvailable {
            target_li: self.highest_known_ledger_info.clone(),
    ...
    fn process_response_with_verifiable_li(
        ...
        if self.sync_request.is_some() {
            match self.create_sync_request_chunk_target(known_version) {
                Ok(chunk_target) => {
                    self.send_chunk_request_and_log_error(known_version, known_epoch, chunk_target);
                }
                Err(error) => {
            ...
            let mut new_highest_known_ledger_info = None;
            if let Some(highest_known_ledger_info) = self.highest_known_ledger_info.clone() {
                if known_version < highest_known_ledger_info.ledger_info().version() {
                    new_highest_known_ledger_info = Some(highest_known_ledger_info);
                }
            }
            let chunk_target = TargetType::HighestAvailable {
                target_li: new_highest_known_ledger_info,
                timeout_ms: self.config.long_poll_timeout_ms,
            };
            self.send_chunk_request_and_log_error(known_version, known_epoch, chunk_target);
        };
        ...
        self.validate_and_store_chunk(txn_list_with_proof, response_li.clone(), None)?;

        // Need to sync with local storage to update synced version
        self.sync_state_with_local_storage()?;
        let synced_version = self.local_state.synced_version();

        // Check if we've synced beyond our current highest known ledger info
        if let Some(highest_known_ledger_info) = &self.highest_known_ledger_info {
            if synced_version >= highest_known_ledger_info.ledger_info().version() {
                self.highest_known_ledger_info = None;
            }
        }

        // If we don't have a highest known ledger info, check if the new highest
        // is appropriate for us.
        if self.highest_known_ledger_info.is_none() {
            if let Some(new_highest_li) = new_highest_li {
                ...
                if synced_version < new_highest_li.ledger_info().version() {
                    self.highest_known_ledger_info = Some(new_highest_li);
                }
            }
        }
    ...
    fn process_response_with_waypoint_li(
        ...
        if known_version < self.waypoint.version() {
            let chunk_target = self.create_waypoint_chunk_target();
            self.send_chunk_request_and_log_error(known_version, known_epoch, chunk_target);
        }
    ...
    fn check_progress(&mut self) -> Result<(), Error> {
        ...
        if self.request_manager.has_request_timed_out(known_version)? {
            ...
            if let Err(error) = self.send_chunk_request(known_version, trusted_epoch) {
                error!(
    ...
    /// Catches and logs any errors returned by the operation.
